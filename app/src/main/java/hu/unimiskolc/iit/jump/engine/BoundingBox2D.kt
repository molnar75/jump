...
import hu.unimiskolc.iit.jump.graph.ShaderProgram

class BoundingBox2D(newMinPoint: hu.unimiskolc.iit.jump.engine.Vector2D, newMaxPoint: hu.unimiskolc.iit.jump.engine.Vector2D) {
    private val aabbPoints2D = 4
    var minpoint: hu.unimiskolc.iit.jump.engine.Vector2D
    var maxpoint: hu.unimiskolc.iit.jump.engine.Vector2D

    private var bbPoints: Array<hu.unimiskolc.iit.jump.engine.Vector2D>
    ...
    init {
        ...
        boxHalfHeight = 0f
        mEnabled = false
        bbPoints = Array(aabbPoints2D) { hu.unimiskolc.iit.jump.engine.Vector2D(0f, 0f) }
        transformationMatrix = FloatArray(16)
        rotationMatrix = FloatArray(16)
        minpoint = hu.unimiskolc.iit.jump.engine.Vector2D(newMinPoint.x, newMinPoint.y)
        maxpoint = hu.unimiskolc.iit.jump.engine.Vector2D(newMaxPoint.x, newMaxPoint.y)
        setUpBBPoints()
        searchMinMax()
    ...
    fun setPoints(min: hu.unimiskolc.iit.jump.engine.Vector2D, max: hu.unimiskolc.iit.jump.engine.Vector2D){
    ...
    private fun searchMinMax() {
        val min = hu.unimiskolc.iit.jump.engine.Vector2D(bbPoints[0].x, bbPoints[0].y)
        val max = hu.unimiskolc.iit.jump.engine.Vector2D(bbPoints[0].x, bbPoints[0].y)
    ...
    private fun transformPoint(vec: hu.unimiskolc.iit.jump.engine.Vector2D){
        val x = vec.x
        val y = vec.y
    ...
    fun transformByTranslate(translateVector: hu.unimiskolc.iit.jump.engine.Vector2D) {
        setIdentityM(transformationMatrix, 0)
    ...
    fun render(shaderProgram: ShaderProgram) {
        ...
        val util = hu.unimiskolc.iit.jump.engine.Utils()
