...
class Sprite(private val context: Context, filenames: String, numOfFrames: Int, var mvSpritePosition: Vector2D, private var miFps: Int) {
    ...
    fun getCurrentFrameTransformedBoundingBox(): hu.unimiskolc.iit.jump.engine.BoundingBox2D {
        val currentFrame = mvFrames[miActualFrame]

        val original: hu.unimiskolc.iit.jump.engine.BoundingBox2D = currentFrame.mBBoxOriginal
        val transformed: hu.unimiskolc.iit.jump.engine.BoundingBox2D = currentFrame.mBBoxTransformed
